...
1. **Revenue per restaurant** — total revenue from delivered orders, by restaurant (JOIN + aggregation)
...
- Apply the `order_status = 'Delivered'` filter consistently to the items-ordered query (currently counts items from cancelled orders too)
- Rider efficiency scoring using multi-condition `CASE` logic (combining average delivery time and total deliveries)
- City-wise ranking using a CTE + `RANK()` combination
- New vs. returning customer revenue split based on signup date vs. order date
